...
- (void)lm_imageSettingsController:(LMImageSettingsController *)viewController insertImage:(UIImage *)image {
    ...
    //上传至服务器
    [RequestGather uploadSingleImage:degradedImage Success:^(id response) {
        NSTextAttachment *attachment = [self insertImage:degradedImage];
        attachment.attachmentType = LMTextAttachmentTypeImage;
        attachment.userInfo = response[@"data"];
        [self.textView becomeFirstResponder];
        [self.textView scrollRangeToVisible:self->_lastSelectedRange];
    } failure:nil];
...
- (NSString *)exportHTML {
    ...
    NSString *exportHTMLStr = [title stringByAppendingString:content];
    GGLog(@"输出h5文本：%@",exportHTMLStr);
    return content;
}
