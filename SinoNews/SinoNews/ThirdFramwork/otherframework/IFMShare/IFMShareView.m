...
//    frame = [UIScreen mainScreen].bounds;
    frame = CGRectMake(0, 0, ScreenW, ScreenH - BOTTOM_MARGIN);
...
-(void)layoutSubviews {
    ...
    //动画前置控件位置
    if (_containView) {
        _containView.frame = CGRectMake(0, SCREEN_HEIGHT - BOTTOM_MARGIN, SCREEN_WIDTH, height);
    }
    if (_headerView) {
    ...
    [UIView animateWithDuration:0.25 animations:^{
        if (_containView) {
            _containView.frame = CGRectMake(0, SCREEN_HEIGHT - height  - BOTTOM_MARGIN, SCREEN_WIDTH, height);
        }
...
- (void)tappedCancel {
    [UIView animateWithDuration:0.25 animations:^{
        ...
        if (self.containView) {
            self.containView.frame = CGRectMake(0, SCREEN_HEIGHT - BOTTOM_MARGIN,SCREEN_WIDTH, _containView.frame.size.height);
        }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.clickBlock) {
        if (collectionView == self.shareCollectionView) {
            ...
            self.clickBlock(1, indexPath.row);
        }
    }
    [self dismiss:YES];
}
