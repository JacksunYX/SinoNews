...
-(void)addBottomBtns
{
    CGFloat btnW = self.buttomView.width/2;
    CGFloat btnH = 48;
    
    self.doneBtn = [self getBtn];
    self.doneBtn.tag = 11;
    self.doneBtn.frame = CGRectMake(0, self.buttomView.height - btnH, btnW, btnH);
    [self.doneBtn setTitle:@"确定" forState:UIControlStateNormal];
    self.doneBtn.layer.borderWidth = 0.5;
    self.doneBtn.layer.borderColor = RGBA(227, 227, 227, 1).CGColor;
    [self.buttomView addSubview:self.doneBtn];
    
    
    self.cancelBtn = [self getBtn];
    self.cancelBtn.tag = 22;
    self.cancelBtn.frame = CGRectMake(btnW, self.buttomView.height - btnH, btnW, btnH);
    [self.cancelBtn setTitle:@"取消" forState:UIControlStateNormal];
    self.cancelBtn.layer.borderWidth = 0.5;
    self.cancelBtn.layer.borderColor = RGBA(227, 227, 227, 1).CGColor;
...
-(UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view {
    ...
    if (self.datePickerStyle == DateStyleShowYearMonthDay) {
        ...
        UIView *customLine = [[UIView alloc]initWithFrame:CGRectMake(20, 0, totalWid, 1)];
        [self drawDashLine:customLine lineLength:avgLineW lineSpacing:space lineColor:RGBA(87, 161, 232, 1)];
        [botomLine addSubview:customLine];
        topLine.hidden = YES;
    ...
    UIView *subview = [pickerView.subviews firstObject];
    
    for ( UIView *pickerColumnView in subview.subviews) {
        UIView *pickerView = [pickerColumnView.subviews lastObject];
        UIView *line = [[UIView alloc]initWithFrame:CGRectMake(0, CGRectGetHeight(pickerView.frame) - 2, 57, 2)];
...
-(void)show {
    ...
    [UIView animateWithDuration:.3 animations:^{
        self.bottomConstraint.constant = keyWindow.center.y - 177/2;
